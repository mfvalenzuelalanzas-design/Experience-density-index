  ...
  var S4 = function() {
    return (
      <div style={{ maxWidth: 510, margin: "0 auto" }}>
        ...
        <div style={{ borderTop: "1px solid #e6e2dc", paddingTop: 12, marginBottom: 12 }}>
          <label style={L}>REFERENCE IMAGES (OPTIONAL)</label>
          <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
            <div style={{ border: "1px dashed #d6d2ca", borderRadius: 2, padding: "7px 12px", cursor: "pointer", fontSize: 10.5, color: "#5a5248" }} onClick={function() { document.getElementById("sk-in").click(); }}>+ Add images</div>
            ...
            {sketches.map(function(sk, i) { return (
              <div key={i} style={{ position: "relative", width: 44, height: 44 }}>
                <img src={sk} alt="" style={{ width: 44, height: 44, objectFit: "cover", borderRadius: 2, border: "1px solid #e2dfd8" }} />
  ...
  var S5 = function() {
    ...
    return (
      <div style={{ maxWidth: 760, margin: "0 auto" }}>
        ...
        {/* 1. ARCHETYPE PENTAGON */}
        <div style={{ marginTop: 16 }}>
          <div style={{ fontSize: 9, letterSpacing: "0.12em", color: "#5a5248", textAlign: "center", marginBottom: 2 }}>DESIGN DNA</div>
        ...
        {/* Archetype + Score */}
        <div style={{ padding: "14px 18px", border: "1px solid #e2dfd8", borderRadius: 3, marginTop: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
            <div>
              <div style={{ fontSize: 8.5, color: "#6a5e4e", letterSpacing: "0.1em" }}>EXPERIENCE ARCHETYPE</div>
              <div style={{ fontSize: 17, color: "#1a1815", fontWeight: 300, marginTop: 1 }}>{arch.name}</div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 8.5, color: "#6a5e4e", letterSpacing: "0.1em" }}>EDI</div>
              <div style={{ fontSize: 22, color: "#1a1815", fontWeight: 300 }}>{aScore}</div>
              <div style={{ fontSize: 9, color: "#4a4238", letterSpacing: "0.08em" }}>{calc.ediTier.toUpperCase()}</div>
            </div>
          </div>
          ...
          <div style={{ borderTop: "1px solid #e6e2dc", marginTop: 8, paddingTop: 6 }}>
        ...
        {/* Experience Emphasis */}
        <div style={{ padding: "10px 14px", border: "1px solid #e2dfd8", borderRadius: 3, marginTop: 8 }}>
          <div style={{ fontSize: 8.5, color: "#6a5e4e", letterSpacing: "0.1em" }}>EXPERIENCE EMPHASIS</div>
          <div style={{ fontSize: 14, color: "#1a1815", fontWeight: 400, marginTop: 2 }}>{calc.expEmphLabel}</div>
        </div>

        {/* Value Proposition */}
        <div style={{ padding: "12px 16px", border: "1px solid #e2dfd8", borderRadius: 3, marginTop: 8, borderLeft: "3px solid #8a7e6e" }}>
          <div style={{ fontSize: 8.5, color: "#6a5e4e", letterSpacing: "0.1em", marginBottom: 3 }}>VALUE PROPOSITION</div>
          <div style={{ fontSize: 12, color: "#1a1815", lineHeight: 1.6, fontStyle: "italic" }}>{calc.valueProposition}</div>
        </div>

        {adr && <div style={{ padding: "8px 14px", border: "1px solid #e2dfd8", borderRadius: 2, marginTop: 8 }}><div style={{ fontSize: 8.5, color: "#6a5e4e", letterSpacing: "0.08em" }}>MARKET ADR</div><div style={{ fontSize: 11, color: "#3a3428", marginTop: 1 }}>{adr}</div></div>}

        {/* 2. EXPERIENCE PROFILE HEXAGON */}
        <div style={{ marginTop: 20 }}>
          <div style={{ fontSize: 9, letterSpacing: "0.12em", color: "#5a5248", textAlign: "center", marginBottom: 2 }}>EXPERIENCE PROFILE</div>
          <Pentagon id="exp" metrics={tpRadar} ghosts={null} labels={CK.map(function(ck) { return CATS[ck]; })} size={320} />
          <div style={{ textAlign: "center", fontSize: 9, color: "#6a5e4e", marginTop: -2 }}>{sTP.size} touchpoints -- weighted by category</div>
        </div>

        {/* 3. BRAND BENCHMARK PENTAGON */}
        {tierData && <div style={{ marginTop: 20 }}>
          <div style={{ fontSize: 9, letterSpacing: "0.12em", color: "#5a5248", textAlign: "center", marginBottom: 2 }}>BRAND POSITIONING -- {tierData.label.toUpperCase()}</div>
        ...
        <div style={{ marginTop: 22 }}>
          <div style={{ fontSize: 9, letterSpacing: "0.12em", color: "#5a5248", marginBottom: 10 }}>KEY METRICS</div>
        ...
        {/* Narrative themes */}
        {calc.narrAnalysis.themes.length > 0 && <div style={{ marginTop: 16, padding: "10px 14px", border: "1px solid #e2dfd8", borderRadius: 2 }}>
          <div style={{ fontSize: 8.5, color: "#6a5e4e", letterSpacing: "0.1em", marginBottom: 4 }}>NARRATIVE THEMES DETECTED</div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>{calc.narrAnalysis.themes.map(function(t, i) { return <span key={i} style={{ padding: "3px 8px", borderRadius: 10, fontSize: 10, border: "1px solid #d6d2ca", color: "#3a3428" }}>{t}</span>; })}</div>
        </div>}

        {/* Touchpoint map */}
        <div style={{ marginTop: 18 }}>
          <div style={{ fontSize: 9, letterSpacing: "0.12em", color: "#5a5248", marginBottom: 6 }}>TOUCHPOINT MAP</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4 }}>
            {Object.entries(CATS).map(function(entry) {
              var ct = calc.tpByCategory[entry[0]] || 0, m = allTP.filter(function(t) { return t.cat === entry[0]; }).length;
              return (
                <div key={entry[0]} style={{ display: "flex", alignItems: "center", gap: 5, padding: "3px 8px", border: "1px solid #e6e2dc", borderRadius: 2 }}>
                  <div style={{ fontSize: 9, color: "#4a4238", width: 52 }}>{entry[1].split(" ")[0]}</div>
                  <div style={{ flex: 1, display: "flex", gap: 2 }}>{Array.from({ length: m }).map(function(_, i) { return <div key={i} style={{ width: 7, height: 7, borderRadius: 1, background: i < ct ? "rgba(0,0,0,0.2)" : "rgba(0,0,0,0.04)" }} />; })}</div>
        ...
        {/* Insights + Recommendations (merged) */}
        {calc.insights.length > 0 && <div style={{ marginTop: 18 }}>
          <div style={{ fontSize: 9, letterSpacing: "0.12em", color: "#5a5248", marginBottom: 8 }}>INSIGHTS & RECOMMENDATIONS</div>
          {calc.insights.map(function(ins, i) { return <div key={i} style={{ padding: "8px 11px", marginBottom: 4, borderRadius: 2, background: "#faf9f7", borderLeft: "2px solid #8a7e6e", fontSize: 11, color: "#2a2420", lineHeight: 1.55 }}>{ins.text}</div>; })}
        </div>}

        {/* PROJECT HIGHLIGHTS */}
        <div style={{ marginTop: 22, padding: "16px 18px", border: "1px solid #d0ccc4", borderRadius: 3, background: "#faf9f7" }}>
          <div style={{ fontSize: 9, letterSpacing: "0.12em", color: "#4a4238", marginBottom: 8 }}>PROJECT HIGHLIGHTS -- WHAT MAKES THIS SPECIAL</div>
        ...
        {/* Sketches */}
        {sketches.length > 0 && <div style={{ marginTop: 16 }}>
          <div style={{ fontSize: 9, letterSpacing: "0.12em", color: "#5a5248", marginBottom: 6 }}>REFERENCE IMAGES</div>
          <div style={{ display: "flex", gap: 6, overflowX: "auto" }}>{sketches.map(function(sk, i) { return <img key={i} src={sk} alt="" style={{ height: 110, borderRadius: 2, border: "1px solid #e2dfd8" }} />; })}</div>
        </div>}
